(() => {
  ...
  function renderPlanning() {
    ...
    $('#planningResults').innerHTML = plan.rows.length ? `<div class="planning-table-scroll"><table><thead><tr>${['planning.part', 'planning.shortage', 'planning.packed', 'planning.inventory', 'planning.pallets', 'planning.required'].map(key => `<th scope="col">${esc(t(key))}</th>`).join('')}</tr></thead><tbody>${plan.rows.map(row => `<tr class="${row.shortage ? 'planning-short' : ''}"><td>${partIdentityMarkup(row.part)}<small>${esc(assemblyLabel(row.part))} · ${esc(dimensionLabel(row.part))}</small><small>${esc(row.projects.join(', '))}</small></td><td><strong>${row.shortage}</strong></td><td>${row.packed}</td><td>${row.inventory}</td><td>${row.pallets}</td><td>${row.required}</td></tr>`).join('')}</tbody></table></div>` : `<p class="empty-state">${esc(t('planning.empty'))}</p>`;
  }
